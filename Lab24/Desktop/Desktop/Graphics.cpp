...
void Graphics::Copy(const void* vertices, ullong sizeInBytes, ID3D12Resource* bufferUpload, ID3D12Resource* bufferGPU)
{
    // ----------------------------------------------------------------------------------
...
void Graphics::Copy(D3D12_SUBRESOURCE_DATA* subresource, ullong sizeInBytes, ID3D12Resource* bufferUpload, ID3D12Resource* bufferGPU)
{
    ...
    for (uint z = 0; z < layouts.Footprint.Depth; ++z)
    {
        ...
        // endereço da fonte
        const BYTE* srcSlice = (const BYTE*)(subresource->pData) + subresource->SlicePitch * z;

        // faz cópia linha a linha
        for (uint y = 0; y < numRows; ++y)
            memcpy(destSlice + DestData.RowPitch * y,
                srcSlice + subresource->RowPitch * y,
    ...
    // --------------------------------------
    // Copia dados do Upload -> GPU Buffer
    // --------------------------------------
