...
{
    // reutiliza alocador de comandos que já foi liberado pela GPU
    // assume que a sincronização com cercas fornece essa garantia
    commandAlloc[bufferIndex]->Reset();

    // uma lista de comandos pode ser reiniciada depois que os 
    // comandos foram adicionados à fila de comandos da GPU 
...
void Graphics::PrepareGpu(ID3D12PipelineState* pso)
{
    // reutiliza alocador de comandos que já foi liberado pela GPU
    // assume que a sincronização com cercas fornece essa garantia
    commandAlloc[bufferIndex]->Reset();
